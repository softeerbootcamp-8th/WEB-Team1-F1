...
import { DealerApplicationsPanel } from './dealer-applications-panel'
...
describe('딜러 자격 심사 목록', () => {
  ...
  it('신청이 없으면 비어 있음을 알린다', async () => {
    ...
    await waitFor(() => expect(screen.getByText('심사 대기 상태의 신청이 없습니다')).toBeTruthy())
  })
  ...
  it('목록을 못 읽으면 실패를 알린다', async () => {
    ...
    await waitFor(() => expect(screen.getByText('신청 목록을 불러오지 못했습니다')).toBeTruthy())
  })
