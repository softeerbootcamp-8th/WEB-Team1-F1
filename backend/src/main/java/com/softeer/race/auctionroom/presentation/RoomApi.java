...
public interface RoomApi {
    ...
    @Operation(summary = "경매방 현황 조회",
            description = "차량 요약, 현재가, 마감 시각, 최근 호가, 접속자 수를 반환합니다. "
                    ...
                    + "보고 있는 사람 수는 조회가 아니라 열려 있는 구독의 회원으로 셉니다. 모든 시각은 KST입니다. "
    ...
    @Operation(summary = "끝난 경매의 결과 요약",
            description = "차량 요약, 시작가와 최종 낙찰가, 낙찰자, 집계, 가격이 오른 과정을 반환합니다. "
                    ...
                    + "입찰가와 순위입니다. 입찰한 적이 없으면 myStanding 이 통째로 null 이라 낙찰자·탈락자·판매자·"
                    + "구경꾼을 이 응답만으로 가릴 수 있습니다. 그래서 세션 쿠키가 필요합니다. "
                    + "더 이상 바뀌지 않는 경매라 접속자 수와 호가창은 담지 않습니다. serverTime 은 결과값이 "
                    + "아니라 남은 열람 시간을 세는 기준이고 resultViewingEndsAt 과 짝입니다. "
