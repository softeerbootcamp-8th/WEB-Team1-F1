...
public interface EvaluationAssignmentApi {
    ...
    @Operation(summary = "배정 대기 목록 조회",
            description = "아직 아무도 수락하지 않은 방문견적 신청을 방문일이 임박한 순서로 돌려줍니다. "
                    ...
                    + "정렬은 sort로 고릅니다 — VISIT_DATE(기본)는 방문일이 임박한 순서, "
                    + "LATEST는 접수가 최근인 순서입니다. "
                    + "커서에 담기는 값은 정렬이 정합니다. VISIT_DATE는 방문일과 신청 ID 두 값이고, "
                    + "LATEST는 신청 ID 하나이며 방문일은 비어 나갑니다. "
                    + "정렬이 요구하는 값을 갖추지 못한 커서는 400입니다 — 정렬을 바꾸면서 이전 커서를 "
                    + "그대로 보내면 여기에 걸리므로, 정렬을 바꿀 때는 커서 없이 첫 페이지부터 읽으십시오. "
                    ...
                    + "건수는 정렬과 무관해 두 정렬이 같은 값을 봅니다. "
                    + "세션 쿠키와 평가사 역할이 필요합니다.")
    @ApiResponse(responseCode = "400",
            description = "커서가 정렬이 요구하는 값을 갖추지 못한 경우입니다.")
