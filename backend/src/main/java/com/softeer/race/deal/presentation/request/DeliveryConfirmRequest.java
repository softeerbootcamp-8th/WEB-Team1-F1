 ...
 */
@Schema(description = "차량 인수 일정 확정")
public record DeliveryConfirmRequest(

        @Schema(description = "차량 인수 일시, 탁송 출발 일시보다 뒤여야 한다",
                example = "2026-08-21T10:00:00")
        @NotNull(message = "인수 일시는 필수입니다.")
        LocalDateTime deliveryAt,

        @Schema(description = "차량 인수 장소", example = "부산시 해운대구 센텀중앙로 55")
        @NotBlank(message = "인수 장소는 필수입니다.")
