...
class BidderStandingTest {
    ...
    private static final long MY_TOP_BID = 9_300_000L;

    @Test
    @DisplayName("나보다 높이 부른 사람이 없으면 1등이다")
    void topBidderIsFirst() {
        assertThat(BidderStanding.of(MY_TOP_BID, 0).rank()).isEqualTo(1);
    }

    @Test
    @DisplayName("나보다 높이 부른 사람 수만큼 순위가 밀린다")
    void rankFollowsBiddersAbove() {
        assertThat(BidderStanding.of(MY_TOP_BID, 1).rank()).isEqualTo(2);
        assertThat(BidderStanding.of(MY_TOP_BID, 3).rank()).isEqualTo(4);
    }
}
