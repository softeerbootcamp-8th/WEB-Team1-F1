...
// 낡은 현황을 거른다는 계약까지만 지킨다
// 밀렸을 때 오래된 것을 버리는 것은 RoomMailboxTest 가, 검사와 전송의 원자성은 BroadcastOrderExperiment 가 본다
class SseRoomSubscriptionTest {

    private static final long AUCTION = 1L;
    private static final long VIEWER = 7L;

    // 부른 자리에서 그대로 돌린다, 밀리는 상황이 아니라 낡음 판정만 보는 테스트라 이게 맞다
    ...
    void staleStateIsNotSent() {
        ...
        RoomSubscription subscription = new SseRoomSubscription(AUCTION, VIEWER, emitter, DIRECT);
